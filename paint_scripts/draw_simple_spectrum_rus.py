...
class Draw_simple_spectrum:
    ...
    def make_dir_name(self,name_dataset:str|None)->str:
        if name_dataset==None:
            return 'plots11/ru/none/'+self.substance_name
        else:
            return 'plots11/ru/'+str(name_dataset)+'/'+self.substance_name
    ...
    def main(self,save:bool=False,name_of_dataset:str|None=None,size_img:tuple=(12,8), label_x_number_size:int=30,
    ...
    def plot(self,save,file_name:str,size_img:tuple,label_x_number_size:int,label_y_number_size:int,pad:int,
        ...
        arr=np.rot90(self.matrix)
        plt.imshow(arr,aspect="auto", origin='upper',extent=[self.data.Emission_wale[0],
                                                                     self.data.Emission_wale[-1],
                                                                     self.data.Exitation_wale[0],
                                                                     self.data.Exitation_wale[-1]])
        plt.xlabel("Регистрация, нм" , fontsize=label_name_fontsize,labelpad=pad)
        plt.ylabel("Возбуждение, нм",  fontsize=label_name_fontsize,labelpad=pad)
